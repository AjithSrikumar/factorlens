...
const CAT_STYLE: Record<string, { bg: string; color: string }> = {
  "Broad Market":  { bg: "rgba(79,128,255,.13)",   color: "#93BFFF" },
  "Momentum":      { bg: "rgba(245,158,11,.13)",   color: "#FCD34D" },
  "Multi-Factor":  { bg: "rgba(139,92,246,.13)",   color: "#C4B5FD" },
  "Quality":       { bg: "rgba(16,185,129,.13)",   color: "#6EE7B7" },
  "Low Vol":       { bg: "rgba(16,185,129,.13)",   color: "#6EE7B7" },
  "Alpha":         { bg: "rgba(236,72,153,.13)",   color: "#F9A8D4" },
  "Value":         { bg: "rgba(251,191,36,.13)",   color: "#FDE68A" },
  "Thematic":      { bg: "rgba(239,68,68,.13)",    color: "#FCA5A5" },
  "Equal Weight":  { bg: "rgba(52,211,153,.13)",   color: "#6EE7B7" },
  "High Beta":     { bg: "rgba(244,63,94,.13)",    color: "#FDA4AF" },
  "Dividend":      { bg: "rgba(251,191,36,.13)",   color: "#FDE68A" },
  "Volatility":    { bg: "rgba(56,189,248,.13)",   color: "#7DD3FC" },
  "Fixed Income":  { bg: "rgba(167,139,250,.13)",  color: "#C4B5FD" },
  "Leverage":      { bg: "rgba(249,115,22,.13)",   color: "#FDBA74" },
  "Global/Other":  { bg: "rgba(148,163,184,.10)",  color: "#94A3B8" },
}

function catStyle(cat: string) {
  return CAT_STYLE[cat] ?? { bg: "rgba(12,14,19,.06)", color: "var(--muted-foreground)" }
}
...
function RankBadge({ rank }: { rank: number | null }) {
  const style = rank === 1
    ...
    : { background: "rgba(148,163,184,.10)", color: "var(--muted-foreground)" }
...
export default function RankingsPage() {
  ...
  function SortArrow({ col }: { col: SortKey }) {
    if (sortKey !== col) return <span style={{ color: "var(--muted-foreground)", fontSize: 9, opacity: 0.4 }}>↕</span>
    return <span style={{ color: "#6B9FFF", fontSize: 9 }}>{sortDir === "asc" ? "↑" : "↓"}</span>
  }
  ...
  function FundCharts({ fund }: { fund: Fund }) {
    ...
    if (chartLoading && expandedId === fund.id && !data) {
      return (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 0", gap: 12 }}>
          <div style={{ width: 24, height: 24, borderRadius: "50%", border: "2.5px solid var(--border)", borderTopColor: "#6B9FFF", animation: "spin .75s linear infinite" }} />
          <span style={{ fontSize: 14, color: "var(--muted-foreground)" }}>Loading charts…</span>
        </div>
      )
    }
    if (!data) return null
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 24, paddingTop: 8 }}>
        <div>
          <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, color: "var(--foreground)" }}>Performance vs Nifty 50</p>
          <p style={{ fontSize: 11, color: "var(--muted-foreground)", marginBottom: 12 }}>₹100 invested at inception</p>
        ...
        <div>
          ...
          <p style={{ fontSize: 11, color: "var(--muted-foreground)", marginBottom: 12 }}>% decline from previous peak</p>
        ...
        {data.fyTableData && (
          <div>
            ...
            <p style={{ fontSize: 11, color: "var(--muted-foreground)", marginBottom: 8 }}>Annual returns vs NIFTY 50</p>
    ...
    <div style={{ minHeight: "100vh", background: "var(--background)" }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{
              ...
              <svg viewBox="0 0 24 24" fill="none" style={{ width: 26, height: 26 }}>
                <path d="M2 14l4-4 3 3 4-5 3 3" stroke="#fbbf24" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <h2 style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-.4px", marginBottom: 10, color: "var(--text-raw)" }}>
              Sign in to view full backtest
            </h2>
            ...
            <button
              ...
              style={{
                ...
                background: "none", color: "var(--text-muted)", fontSize: 13, fontWeight: 500,
      ...
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "32px 32px 64px" }} className="rank-wrap-resp">
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div>
              ...
              {!loading && lastNavDate && (() => {
                ...
                return (
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 6, padding: "3px 10px", borderRadius: 99, fontSize: 11.5, fontWeight: 600, background: isStale ? "rgba(239,68,68,.10)" : "rgba(16,185,129,.10)", color: isStale ? "#F87171" : "#34D399" }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: isStale ? "#F87171" : "#34D399", display: "inline-block", flexShrink: 0 }} />
                    {isStale ? `Data stale · last updated ${lastNavDate}` : `Data as of ${lastNavDate}`}
                  </div>
                )
              })()}
            </div>
            <Link href="/dashboard" className="fl-btn-primary" style={{ padding: "9px 18px", fontSize: 13, borderRadius: 9 }}>
              Build Portfolio
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 10.5, color: "var(--muted-foreground)", fontWeight: 600, marginTop: 1 }}>
                        {f.category}
                      </div>
                    </div>
                  </div>

                  <div style={{ fontSize: 13.5, fontWeight: 700, letterSpacing: "-.1px", marginBottom: 12, lineHeight: 1.35, color: "var(--foreground)" }}>
                    {f.name}
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div style={{ background: "rgba(255,255,255,.6)", borderRadius: 10, padding: "9px 11px" }}>
                      <div style={{
                        fontFamily: "var(--font-serif, 'Instrument Serif', Georgia, serif)",
                        fontSize: 19, fontWeight: 400, color: "#34D399", letterSpacing: "-.3px",
                      }}>
                        {pct(f.cagr)}
                      </div>
                      <div style={{ fontSize: 9.5, color: "var(--muted-foreground)", fontWeight: 700, letterSpacing: ".6px", textTransform: "uppercase" as const, marginTop: 1 }}>
                        CAGR
                      </div>
                    </div>
                    <div style={{ background: "rgba(255,255,255,.6)", borderRadius: 10, padding: "9px 11px" }}>
                      <div style={{
                        fontFamily: "var(--font-serif, 'Instrument Serif', Georgia, serif)",
                        fontSize: 19, fontWeight: 400, color: "var(--foreground)", letterSpacing: "-.3px",
                      }}>
        ...
        <div style={{
          ...
          <div style={{
            width: 28, height: 28, borderRadius: 8, background: "rgba(79,128,255,.12)",
          ...
          <p style={{ fontSize: 13, color: "var(--muted-foreground)", lineHeight: 1.6 }}>
            <strong style={{ color: "var(--foreground)" }}>Score out of 10</strong> — composite of 20Y CAGR (30%), 3Y rolling return (25%), Sharpe ratio (30%), and drawdown protection (15%). Higher is better.{" "}
            Only indices with <strong style={{ color: "var(--foreground)" }}>at least 10 years of history</strong> are ranked.{" "}
            <strong style={{ color: "var(--foreground)" }}>Tap any row</strong> to expand.{" "}
            <strong style={{ color: "var(--foreground)" }}>Fund name →</strong> opens full detail page.
          </p>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start", flexWrap: "wrap" as const, marginBottom: 20 }}
          ...
          <div style={{ position: "relative", flex: "0 0 auto", width: "100%", maxWidth: 290, minWidth: 0 }} className="rfilt-search">
            <svg viewBox="0 0 20 20" fill="none" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", width: 15, height: 15, color: "var(--muted-foreground)", pointerEvents: "none" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "var(--background)" }}>
                      ...
                      <th style={{ padding: "10px 16px", borderBottom: "1px solid var(--border)", width: 32 }} />
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        >
                          ...
                          <td style={{ padding: "13px 16px", maxWidth: 260, fontSize: 13.5, verticalAlign: "middle" }}>
                            <Link
                              ...
                              style={{ fontWeight: 600, letterSpacing: "-.1px", color: "var(--foreground)", textDecoration: "none", lineHeight: 1.35 }}
                          ...
                          <td style={{ padding: "13px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--muted-foreground)", verticalAlign: "middle" }}>
                          ...
                          <td style={{ padding: "13px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "#F87171", verticalAlign: "middle" }}>
                          ...
                          <td style={{ padding: "13px 16px", verticalAlign: "middle" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 90 }}>
                              ...
                              <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 700, color: "var(--foreground)", whiteSpace: "nowrap" as const }}>
                                {displayScore(fund.score)} / 10
                              </span>
                            </div>
                          </td>
                          <td style={{ padding: "13px 16px", color: "var(--muted-foreground)", verticalAlign: "middle" }}>
                            ...
                            <td colSpan={9} style={{ padding: "22px 24px" }}>
                              <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
                                <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid rgba(12,14,19,.08)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                                  ...
                                  <span style={{ display: "inline-flex", padding: "3px 9px", borderRadius: 100, fontSize: 11, fontWeight: 600, background: "rgba(12,14,19,.06)", color: "var(--muted-foreground)" }}>vs Nifty 50</span>
                                </div>
          ...
          <div className="max-md:hidden" style={{ marginTop: 32 }}>
            ...
            <button
              ...
              style={{
                ...
                background: "var(--card)", border: "1px solid rgba(12,14,19,.10)",
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{
                  ...
                  background: "rgba(12,14,19,.06)", color: "var(--muted-foreground)",
                }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <svg style={{ width: 18, height: 18, color: "var(--muted-foreground)", transition: "transform .22s ease", transform: unrankedExpanded ? "rotate(180deg)" : "none" }} viewBox="0 0 18 18" fill="none">
                  <path d="M4.5 7l4.5 4.5 4.5-4.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </button>
            ...
            {unrankedExpanded && (
              <div style={{ background: "var(--card)", border: "1px solid rgba(12,14,19,.10)", borderTop: "none", borderRadius: "0 0 16px 16px", overflow: "hidden" }}>
                <p style={{ fontSize: 12, color: "var(--muted-foreground)", padding: "8px 18px 10px", borderBottom: "1px solid rgba(12,14,19,.06)" }}>
                        ...
                        <>
                          ...
                          >
                            <td style={{ padding: "12px 16px", maxWidth: 260, fontSize: 13.5, verticalAlign: "middle" }}>
                              <Link href={`/rankings/${fund.id}`} style={{ fontWeight: 600, color: "var(--foreground)", textDecoration: "none", lineHeight: 1.35 }} className="hover:!text-[#6B9FFF]" onClick={e => e.stopPropagation()}>
                                {toTitleCase(fund.name)}
                              </Link>
                              <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted-foreground)", marginTop: 2, opacity: 0.6 }}>{fund.code}</div>
                            </td>
                            <td style={{ padding: "12px 16px", verticalAlign: "middle" }}>
                              <span style={{ display: "inline-flex", padding: "3px 9px", borderRadius: 100, fontSize: 11, fontWeight: 600, ...catStyle(fund.category) }}>{fund.category}</span>
                            </td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, verticalAlign: "middle", fontWeight: 700, color: (fund.cagr_10y ?? 0) > 0.15 ? "#34D399" : "var(--foreground)" }}>{pct(fund.cagr_10y)}</td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--muted-foreground)", verticalAlign: "middle" }}>{pct(fund.avg_3y_rolling_return)}</td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, verticalAlign: "middle" }}>
                              <span style={{ color: (fund.sharpe_ratio ?? 0) > 0.6 ? "#6B9FFF" : "var(--foreground)" }}>{fixed(fund.sharpe_ratio)}</span>
                            </td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "#F87171", verticalAlign: "middle" }}>{pct(fund.max_drawdown)}</td>
                            <td style={{ padding: "12px 16px", color: "var(--muted-foreground)", verticalAlign: "middle" }}>
                              <svg style={{ width: 18, height: 18, transition: "transform .22s ease", transform: expandedId === fund.id ? "rotate(180deg)" : "none" }} viewBox="0 0 18 18" fill="none">
                                <path d="M4.5 7l4.5 4.5 4.5-4.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                              </svg>
                            </td>
                          ...
                          {expandedId === fund.id && (
                            <tr key={`${fund.id}-charts`} style={{ borderBottom: "1px solid rgba(79,128,255,.12)", background: "rgba(79,128,255,.03)" }}>
                              <td colSpan={7} style={{ padding: "22px 24px" }}>
                                <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
                                  <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                                    <span style={{ fontWeight: 700, fontSize: 14, color: "var(--foreground)" }}>{toTitleCase(fund.name)}</span>
                                    <span style={{ display: "inline-flex", padding: "3px 9px", borderRadius: 100, fontSize: 11, fontWeight: 600, ...catStyle(fund.category) }}>{fund.category}</span>
                                  </div>
        ...
        {/* Mobile Cards */}
        <div className="md:hidden" style={{ border: "1px solid var(--border)", borderRadius: 20, overflow: "hidden", background: "var(--card)" }}>
          {loading ? (
            <div style={{ padding: "48px 0", display: "flex", justifyContent: "center" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid var(--border)", borderTopColor: "#6B9FFF", animation: "spin .75s linear infinite" }} />
            </div>
          ) : (
            rankedSorted.map((fund) => (
              <div
              ...
              >
                ...
                <div
                  style={{
                    ...
                    background: expandedId === fund.id ? "rgba(79,128,255,.04)" : undefined,
                    borderBottom: "1px solid var(--border)",
                  }}
                  onClick={() => handleToggleExpand(fund.id)}
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <Link
                      href={`/rankings/${fund.id}`}
                      style={{ fontSize: 14, fontWeight: 700, display: "block", letterSpacing: "-.1px", color: "var(--foreground)", textDecoration: "none", lineHeight: 1.35 }}
                      onClick={e => e.stopPropagation()}
                    >
                      {toTitleCase(fund.name)}
                    </Link>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5 }}>
                      ...
                      <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted-foreground)", background: "rgba(148,163,184,.10)", padding: "1px 6px", borderRadius: 4 }}>
                        {fund.code}
                      </span>
                    </div>
                  </div>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{
                      fontFamily: "var(--font-serif, 'Instrument Serif', Georgia, serif)",
                      fontSize: 22, fontWeight: 400, color: "#34D399", letterSpacing: "-.3px",
                    }}>
                      {pct(fund.cagr)}
                    </div>
                    {nifty50 && fund.id !== nifty50.id && fund.cagr != null && nifty50.cagr != null && (
                      <div style={{
                        ...
                        color: fund.cagr > nifty50.cagr ? "#34D399" : "#F87171",
                      }}>
                  ...
                  <svg
                    style={{ width: 18, height: 18, color: "var(--muted-foreground)", transition: "transform .22s ease", transform: expandedId === fund.id ? "rotate(180deg)" : "none", flexShrink: 0 }}
                ...
                {/* Expanded content */}
                {expandedId === fund.id && (
                  <div style={{ borderTop: "1px solid var(--border)", background: "rgba(79,128,255,.03)" }}>
                    {/* 10Y CAGR highlight */}
                    <div style={{ margin: "14px 20px 0", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", background: "var(--card)", padding: "12px 15px" }}>
                      <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: ".7px", textTransform: "uppercase" as const, color: "var(--muted-foreground)", marginBottom: 4, opacity: 0.6 }}>10-Year CAGR</div>
                      <div style={{ fontFamily: "var(--font-mono)", fontSize: 20, fontWeight: 700, color: (fund.cagr_10y ?? 0) > 0.15 ? "#34D399" : "var(--foreground)" }}>{pct(fund.cagr_10y)}</div>
                    </div>
                    {/* 2×2 metrics grid */}
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", margin: "10px 20px", background: "var(--card)" }}>
                      {[
                        ...
                        <div key={m.label} style={{
                          ...
                          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".8px", textTransform: "uppercase" as const, color: "var(--muted-foreground)", marginBottom: 5, opacity: 0.6 }}>
                            {m.label}
                          </div>
                          <div style={{
                            fontFamily: "var(--font-serif, 'Instrument Serif', Georgia, serif)",
                            fontSize: 20, fontWeight: 400, letterSpacing: "-.2px",
                            color: m.red ? "#F87171" : "var(--foreground)",
                          }}>
                            {m.value}
                          </div>
                        </div>
                    ...
                    <div style={{ padding: "0 20px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".8px", textTransform: "uppercase" as const, color: "var(--muted-foreground)" }}>Composite Score</span>
                        <span style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 700, color: "#6B9FFF" }}>{displayScore(fund.score)} / 10</span>
                      </div>
                    ...
                    <div style={{ borderTop: "1px dashed rgba(12,14,19,.12)", padding: 20 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".8px", textTransform: "uppercase" as const, color: "var(--muted-foreground)" }}>Performance vs Nifty 50</p>
                        <Link
                          href={`/rankings/${fund.id}`}
                          onClick={e => e.stopPropagation()}
                          style={{
        ...
        {!loading && fixedIncomeSorted.length > 0 && (
          <div className="max-md:hidden" style={{ marginTop: 32 }}>
            ...
            {fixedIncomeExpanded && (
              <div style={{ background: "var(--card)", border: "1.5px solid #DDD6FE", borderTop: "none", borderRadius: "0 0 16px 16px", overflow: "hidden" }}>
                {/* Info note */}
                      ...
                      {fixedIncomeSorted.map((fund) => (
                        <>
                          ...
                          >
                            ...
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 700, color: "#34D399", verticalAlign: "middle" }}>{pct(fund.cagr)}</td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--muted-foreground)", verticalAlign: "middle" }}>{pct(fund.cagr_1y)}</td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--muted-foreground)", verticalAlign: "middle" }}>{pct(fund.cagr_3y)}</td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--muted-foreground)", verticalAlign: "middle" }}>{pct(fund.cagr_5y)}</td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, verticalAlign: "middle" }}>
                              <span style={{ color: (fund.sharpe_ratio ?? 0) > 0.4 ? "#1A56DB" : "#0C0E13" }}>{fixed(fund.sharpe_ratio)}</span>
                            </td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--muted-foreground)", verticalAlign: "middle" }}>{pct(fund.volatility)}</td>
                            <td style={{ padding: "12px 16px", fontFamily: "var(--font-mono)", fontSize: 13, color: "#F87171", verticalAlign: "middle" }}>{pct(fund.max_drawdown)}</td>
                          ...
                          {expandedId === fund.id && (
                            <tr key={`${fund.id}-fi-charts`} style={{ borderBottom: "1px solid rgba(91,33,182,.15)", background: "rgba(237,233,254,.25)" }}>
                              <td colSpan={9} style={{ padding: "22px 24px" }}>
                                <div style={{ background: "var(--card)", border: "1px solid rgba(91,33,182,.15)", borderRadius: 16, overflow: "hidden" }}>
        ...
        {/* Footer note */}
        <p style={{ fontSize: 11.5, color: "var(--muted-foreground)", textAlign: "center", marginTop: 18, lineHeight: 1.6 }}>
        ...
        {!loading && unrankedSorted.length > 0 && (
          <div className="md:hidden" style={{ marginTop: 28 }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "var(--foreground)" }}>Not Ranked — Insufficient History</span>
                <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 100, background: "rgba(12,14,19,.06)", color: "var(--muted-foreground)" }}>
                  {unrankedSorted.length}
                </span>
              </div>
              <svg style={{ width: 18, height: 18, color: "var(--muted-foreground)", transition: "transform .22s ease", transform: unrankedExpanded ? "rotate(180deg)" : "none", flexShrink: 0 }} viewBox="0 0 18 18" fill="none">
                <path d="M4.5 7l4.5 4.5 4.5-4.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
              ...
              <div style={{ border: "1px solid rgba(12,14,19,.10)", borderTop: "none", borderRadius: "0 0 16px 16px", overflow: "hidden", background: "#ffffff" }}>
                {unrankedSorted.map(fund => (
                  <div key={fund.id} style={{ borderBottom: "1px solid rgba(12,14,19,.08)", padding: "14px 18px", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <Link href={`/rankings/${fund.id}`} style={{ fontSize: 13.5, fontWeight: 700, color: "var(--foreground)", textDecoration: "none", lineHeight: 1.35 }}>{toTitleCase(fund.name)}</Link>
                      <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                        ...
                        <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted-foreground)", background: "rgba(12,14,19,.06)", padding: "1px 6px", borderRadius: 4 }}>{fund.code}</span>
                      </div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      ...
                      <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted-foreground)", marginTop: 2 }}>10Y CAGR</div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {fixedIncomeSorted.map(fund => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        <div style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 700, color: "#34D399" }}>{pct(fund.cagr)}</div>
                    ...
                    {expandedId === fund.id && (
                      <div style={{ borderTop: "1px solid rgba(91,33,182,.1)", background: "#F5F3FF", padding: 16 }}>
                        {/* Metrics grid */}
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16, background: "var(--card)", border: "1px solid rgba(91,33,182,.1)", borderRadius: 10, overflow: "hidden" }}>
                          {[
